import { deepOrange, green } from 'dalya-components/colors';
import { createPalette } from './createPalette';
import createTheme from './createTheme';

describe('createTheme', () => {
  it('returns default palette if no custom palette object provided', () => {
    const themePalette = createTheme().palette;
    const defaultPalette = createPalette({});
    expect(themePalette.background).toEqual(defaultPalette.background);
  });
  ...
  describe('transitions', () => {
    it('returns default easing values in theme.transitions.easing object', () => {
      const theme = createTheme();

      expect(theme.transitions.easing.standard).toBe('cubic-bezier(0.4, 0, 0.2, 1)');
      expect(theme.transitions.easing.decelerated).toBe('cubic-bezier(0, 0, 0.2, 1)');
      expect(theme.transitions.easing.accelerated).toBe('cubic-bezier(0.4, 0, 1, 1)');
      expect(theme.transitions.easing.sharp).toBe('cubic-bezier(0.4, 0, 0.6, 1)');
    });

    it('returns default duration values in theme.transitions.duration object', () => {
      const theme = createTheme();
    ...
    it('can assign custom easing values', () => {
      const theme = createTheme({
        transitions: {
          easing: {
            standard: 'cubic-bezier(1, 1, 1, 1)',
            decelerated: 'cubic-bezier(1, 1, 1, 1)',
            accelerated: 'cubic-bezier(1, 1, 1, 1)',
            sharp: 'cubic-bezier(1, 1, 1, 1)',
          },
        },
      });
      expect(theme.transitions.easing.standard).toBe('cubic-bezier(1, 1, 1, 1)');
      expect(theme.transitions.easing.decelerated).toBe('cubic-bezier(1, 1, 1, 1)');
      expect(theme.transitions.easing.accelerated).toBe('cubic-bezier(1, 1, 1, 1)');
  ...
  describe('shadows', () => {
    it('returns default shadow object', () => {
      const theme = createTheme();
